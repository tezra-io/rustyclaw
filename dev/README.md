# RustyClaw Development Environment

A fully containerized development sandbox for RustyClaw agents. This environment allows you to develop, test, and debug the agent in isolation without modifying your host system.
...
### 2. Enter Agent Container (`rustyclaw-dev`)

```bash
./dev/cli.sh agent
```

Use this to run `rustyclaw` CLI commands manually, debug the binary, or check logs internally.
...
- **Agent**: `/rustyclaw-data/workspace`
- **Sandbox**: `/home/developer/workspace`

Files created by the agent are visible to the sandbox user, and vice versa.

The agent configuration lives in `target/.rustyclaw` (mounted to `/rustyclaw-data/.rustyclaw`), so settings persist across container rebuilds.
...
**Note:** This removes `target/.rustyclaw` (config/DB) but leaves the `playground/` directory intact. To fully wipe everything, manually delete `playground/`.
...
- Rust compilation, tests, and audit/deny tools run in `rustyclaw-local-ci` container.
...
- The root `Dockerfile` also caches Rust `target/` (`id=rustyclaw-target`) to speed repeat local image builds.
