...
impl ListenTool {
    pub fn new(config: RobotConfig) -> Self {
        let recordings_dir = directories::UserDirs::new()
            .map(|d| d.home_dir().join(".rustyclaw/recordings"))
            .unwrap_or_else(|| PathBuf::from("/tmp/rustyclaw_recordings"));
    ...
    async fn transcribe(&self, audio_path: &Path) -> Result<String> {
        ...
        // whisper.cpp model path (typically in ~/.rustyclaw/models/)
        let model_path = directories::UserDirs::new()
            .map(|d| {
                d.home_dir()
                    .join(format!(".rustyclaw/models/ggml-{}.bin", model))
            })
            .unwrap_or_else(|| {
