...
A complete toolkit for building AI-powered robots with RustyClaw. Designed for Raspberry Pi deployment with offline Ollama inference.
...
### 1. Build RustyClaw with robot tools

```bash
# Clone and build
git clone https://github.com/tezra-io/rustyclaw
cd rustyclaw
cargo build -p rustyclaw-robot-kit --release
```
...
# Copy config
mkdir -p ~/.rustyclaw
cp crates/robot-kit/robot.toml ~/.rustyclaw/
cp crates/robot-kit/SOUL.md ~/.rustyclaw/workspace/
...
# Test in mock mode
./target/release/rustyclaw agent -m "Say hello and show a happy face"

# Test with real hardware
# (after configuring robot.toml)
./target/release/rustyclaw agent -m "Move forward 1 meter"
```
...
```rust
use rustyclaw_robot_kit::{create_tools, RobotConfig};
...
If you want runtime registration in `rustyclaw`, add a thin adapter that maps this
...
mkdir rustyclaw-robot-kit
cp -r target/release/rustyclaw rustyclaw-robot-kit/
cp -r examples/robot_kit rustyclaw-robot-kit/
cp -r ~/.rustyclaw rustyclaw-robot-kit/dot-rustyclaw

# Include models
mkdir -p rustyclaw-robot-kit/models
cp ~/.rustyclaw/models/ggml-base.bin rustyclaw-robot-kit/models/
# Note: Ollama models are large, may want to download on target

# Create tarball
tar -czvf rustyclaw-robot-kit.tar.gz rustyclaw-robot-kit/

# Copy to USB
cp rustyclaw-robot-kit.tar.gz /media/usb/TarBalls/
```
