...
Complete guide to setting up a RustyClaw-powered robot on Raspberry Pi 5.
...
cp models/ggml-base.bin ~/.rustyclaw/models/
```
...
mkdir -p ~/.rustyclaw/models/piper
cd ~/.rustyclaw/models/piper
wget https://huggingface.co/rhasspy/piper-voices/resolve/main/en/en_US/lessac/medium/en_US-lessac-medium.onnx
wget https://huggingface.co/rhasspy/piper-voices/resolve/main/en/en_US/lessac/medium/en_US-lessac-medium.onnx.json

# Test
echo "Hello, I am your robot!" | piper --model ~/.rustyclaw/models/piper/en_US-lessac-medium.onnx --output_file test.wav
aplay test.wav
```
...
### 7. Build RustyClaw Robot Kit

```bash
# Clone repo (or copy from USB)
git clone https://github.com/theonlyhennygod/rustyclaw
cd rustyclaw

# Build robot kit
cargo build --release -p rustyclaw-robot-kit

# Build main rustyclaw (optional, if using as agent)
cargo build --release
```
...
```bash
mkdir -p ~/.rustyclaw
nano ~/.rustyclaw/robot.toml
```

```toml
# ~/.rustyclaw/robot.toml - Real Hardware Configuration
...
# Test speaker
echo "Testing speaker" | piper --model ~/.rustyclaw/models/piper/en_US-lessac-medium.onnx --output_file - | aplay -D plughw:0,0
...
FIFO_PATH = "/tmp/rustyclaw_sensors.fifo"
...
### Start RustyClaw Agent

```bash
# Configure RustyClaw to use robot tools
...
cp ~/rustyclaw/crates/robot-kit/SOUL.md ~/.rustyclaw/workspace/

# Start agent
./target/release/rustyclaw agent
```
...
# Start sensor loop
if [ ! -p /tmp/rustyclaw_sensors.fifo ]; then
    mkfifo /tmp/rustyclaw_sensors.fifo
fi
...
# Start rustyclaw
cd ~/rustyclaw
./target/release/rustyclaw daemon &
AGENT_PID=$!
...
```bash
# /etc/systemd/system/rustyclaw-robot.service
sudo tee /etc/systemd/system/rustyclaw-robot.service << 'EOF'
[Unit]
Description=RustyClaw Robot
...
sudo systemctl enable rustyclaw-robot
sudo systemctl start rustyclaw-robot

# Check status
sudo systemctl status rustyclaw-robot
journalctl -u rustyclaw-robot -f  # View logs
```
