# Adding Boards and Tools — RustyClaw Hardware Guide

This guide explains how to add new hardware boards and custom tools to RustyClaw.

## Quick Start: Add a Board via CLI

```bash
# Add a board (updates ~/.rustyclaw/config.toml)
rustyclaw peripheral add nucleo-f401re /dev/ttyACM0
rustyclaw peripheral add arduino-uno /dev/cu.usbmodem12345
rustyclaw peripheral add rpi-gpio native   # for Raspberry Pi GPIO (Linux)
...
With the `rag-pdf` feature, RustyClaw can index PDF files:
...
2. **Add to config** — `rustyclaw peripheral add my-board /dev/ttyUSB0`
...
| Command | Description |
|---------|-------------|
| `rustyclaw peripheral list` | List configured boards |
| `rustyclaw peripheral add <board> <path>` | Add board (writes config) |
| `rustyclaw peripheral flash` | Flash Arduino firmware |
| `rustyclaw peripheral flash-nucleo` | Flash Nucleo firmware |
| `rustyclaw hardware discover` | List USB devices |
| `rustyclaw hardware info` | Chip info via probe-rs |
