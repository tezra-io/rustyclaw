...
| **Matrix sync (including E2EE)** | No | RustyClaw syncs via Matrix client API; no inbound webhook required |
...
**Key:** Telegram, Discord, Slack, and Nostr use **outbound connections** — RustyClaw connects to external servers/relays. No port forwarding or public IP required.
...
```bash
rustyclaw daemon --host 127.0.0.1 --port 42617
```
...
```bash
rustyclaw daemon --host 0.0.0.0 --port 42617
```
...
- Keep one runtime instance for the same token (recommended: `rustyclaw daemon` service).
...
- [ ] Run `rustyclaw daemon --host 127.0.0.1 --port 42617` (Telegram works without 0.0.0.0)
...
RustyClaw supports OpenRC for Alpine Linux and other distributions using the OpenRC init system. OpenRC services run **system-wide** and require root/sudo.

### 7.1 Prerequisites

- Alpine Linux (or another OpenRC-based distro)
- Root or sudo access
- A dedicated `rustyclaw` system user (created during install)
...
This creates:
- Init script: `/etc/init.d/rustyclaw`
- Config directory: `/etc/rustyclaw/`
- Log directory: `/var/log/rustyclaw/`

### 7.3 Configuration

Manual config copy is usually not required.

`sudo rustyclaw service install` automatically prepares `/etc/rustyclaw`, migrates existing runtime state from your user setup when available, and sets ownership/permissions for the `rustyclaw` service user.

If no prior runtime state is available to migrate, create `/etc/rustyclaw/config.toml` before starting the service.
...
# Check status
sudo rc-service rustyclaw status
```
...
| Command | Description |
|---------|-------------|
| `sudo rc-service rustyclaw start` | Start the daemon |
| `sudo rc-service rustyclaw stop` | Stop the daemon |
| `sudo rc-service rustyclaw status` | Check service status |
| `sudo rc-service rustyclaw restart` | Restart the daemon |
| `sudo rustyclaw service status` | RustyClaw status wrapper (uses `/etc/rustyclaw` config) |
...
| Log | Path |
|-----|------|
| Access/stdout | `/var/log/rustyclaw/access.log` |
| Errors/stderr | `/var/log/rustyclaw/error.log` |

View logs:

```bash
sudo tail -f /var/log/rustyclaw/error.log
```
...
sudo rc-service rustyclaw stop
sudo rc-update del rustyclaw default

# Remove init script
sudo rustyclaw service uninstall
```
...
- The service runs as the `rustyclaw:rustyclaw` user (least privilege)
- Config must be at `/etc/rustyclaw/config.toml` (explicit path in init script)
- If the `rustyclaw` user does not exist, install will fail with instructions to create it

### 7.9 Checklist: Alpine/OpenRC Deployment

- [ ] Install: `sudo rustyclaw service install`
- [ ] Enable: `sudo rc-update add rustyclaw default`
- [ ] Start: `sudo rc-service rustyclaw start`
- [ ] Verify: `sudo rc-service rustyclaw status`
- [ ] Check logs: `/var/log/rustyclaw/error.log`
