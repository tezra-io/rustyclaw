...
RustyClaw supports custom API endpoints for both OpenAI-compatible and Anthropic-compatible providers.
...
# or: export RUSTYCLAW_API_KEY="your-api-key"
rustyclaw agent
```

## llama.cpp Server (Recommended Local Setup)

RustyClaw includes a first-class local provider for `llama-server`:
...
```bash
rustyclaw models refresh --provider llamacpp
rustyclaw agent -m "hello"
```

You do not need to export `RUSTYCLAW_API_KEY=dummy` for this flow.

## SGLang Server

RustyClaw includes a first-class local provider for [SGLang](https://github.com/sgl-project/sglang):
...
```bash
rustyclaw models refresh --provider sglang
rustyclaw agent -m "hello"
```
...
RustyClaw includes a first-class local provider for [vLLM](https://docs.vllm.ai/):
...
```bash
rustyclaw models refresh --provider vllm
rustyclaw agent -m "hello"
```
...
# Single message test
rustyclaw agent -m "test message"
```
