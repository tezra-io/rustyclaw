...
- CLI `rustyclaw sop` currently manages definitions only: `list`, `validate`, `show`.
...
2. Create a SOP directory, for example:
   ...
   ```text
   ~/.rustyclaw/workspace/sops/deploy-prod/SOP.toml
   ~/.rustyclaw/workspace/sops/deploy-prod/SOP.md
   ```
...
3. Validate and inspect definitions:
   ...
   ```bash
   rustyclaw sop list
   rustyclaw sop validate
   rustyclaw sop show deploy-prod
   ```
