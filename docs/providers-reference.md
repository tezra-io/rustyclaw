...
3. Generic fallback env vars: `RUSTYCLAW_API_KEY` then `API_KEY`
...
- After multimodal normalization, RustyClaw sends image payloads through Ollama's native `messages[].images` field.
- If a non-vision provider is selected, RustyClaw returns a structured capability error instead of silently ignoring images.
...
- RustyClaw normalizes a trailing `/api` in `api_url` automatically.
...
- Model discovery: `rustyclaw models refresh --provider llamacpp`
...
- Model discovery: `rustyclaw models refresh --provider sglang`
...
- Model discovery: `rustyclaw models refresh --provider vllm`
...
- Model discovery: `rustyclaw models refresh --provider osaurus`
...
- Model discovery: `rustyclaw models refresh --provider nvidia`
