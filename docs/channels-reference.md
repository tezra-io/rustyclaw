...
This document is the canonical reference for channel configuration in RustyClaw.
...
6. **Stale runtime state**: config changed but `rustyclaw daemon` was not restarted.

---

## 1. Configuration Namespace

All channel settings live under `channels_config` in `~/.rustyclaw/config.toml`.
...
When running `rustyclaw channel start` (or daemon mode), Telegram and Discord now support sender-scoped runtime switching:
...
- Model cache previews come from `rustyclaw models refresh --provider <ID>`.
- These are runtime chat commands, not CLI subcommands.

## Inbound Image Marker Protocol

RustyClaw supports multimodal input through inline message markers:
...
If `[channels_config.matrix]`, `[channels_config.lark]`, or `[channels_config.feishu]` is present but the corresponding feature is not compiled in, `rustyclaw channel list`, `rustyclaw channel doctor`, and `rustyclaw channel start` will report that the channel is intentionally skipped for this build.
...
access_token = "syt_..."
user_id = "@rustyclaw:matrix.example.com"   # optional, recommended for E2EE
...
- `RUSTYCLAW_NEXTCLOUD_TALK_WEBHOOK_SECRET` overrides config secret.
...
- `RUSTYCLAW_LINQ_SIGNING_SECRET` overrides config secret.
...
5. Restart `rustyclaw daemon` after config changes.
...
```bash
rg -n "Matrix|Telegram|Discord|Slack|Mattermost|Signal|WhatsApp|Email|IRC|Lark|DingTalk|QQ|iMessage|Nostr|Webhook|Channel" /tmp/rustyclaw.log
```
