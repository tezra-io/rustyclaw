# RustyClaw trên Arduino Uno Q — Hướng dẫn từng bước

Chạy RustyClaw trên phía Linux của Arduino Uno Q. Telegram hoạt động qua WiFi; điều khiển GPIO dùng Bridge (yêu cầu một ứng dụng App Lab tối giản).

---

## Những gì đã có sẵn (Không cần thay đổi code)

RustyClaw bao gồm mọi thứ cần thiết cho Arduino Uno Q. **Clone repo và làm theo hướng dẫn này — không cần patch hay code tùy chỉnh nào.**

| Thành phần | Vị trí | Mục đích |
|------------|--------|---------|
| Bridge app | `firmware/rustyclaw-uno-q-bridge/` | MCU sketch + Python socket server (port 9999) cho GPIO |
| Bridge tools | `src/peripherals/uno_q_bridge.rs` | Tool `gpio_read` / `gpio_write` giao tiếp với Bridge qua TCP |
| Setup command | `src/peripherals/uno_q_setup.rs` | `rustyclaw peripheral setup-uno-q` triển khai Bridge qua scp + arduino-app-cli |
...
## Phase 2: Cài đặt RustyClaw trên Uno Q
...
# Clone rustyclaw (hoặc scp project của bạn)
...
**Tại bước này:** Telegram chat hoạt động. Gửi tin nhắn tới bot — RustyClaw phản hồi. Chưa có GPIO.

---

## Phase 5: GPIO qua Bridge (RustyClaw xử lý tự động)

RustyClaw bao gồm Bridge app và setup command.

### 5.1 Triển khai Bridge App

**Từ Mac** (với repo rustyclaw):
```bash
...
Lệnh này copy Bridge app vào `~/ArduinoApps/rustyclaw-uno-q-bridge` và khởi động nó.
...
Giờ khi bạn nhắn tin cho Telegram bot *"Turn on the LED"* hoặc *"Set pin 13 high"*, RustyClaw dùng `gpio_write` qua Bridge.
...
| 8 | Chỉnh sửa `~/.rustyclaw/config.toml` (thêm Telegram bot_token) |
| 9 | `rustyclaw daemon --host 127.0.0.1 --port 3000` |
...
- **"command not found: rustyclaw"** — Dùng đường dẫn đầy đủ: `/usr/local/bin/rustyclaw` hoặc đảm bảo `~/.cargo/bin` nằm trong PATH.
...
- **Lệnh GPIO bị bỏ qua** — Đảm bảo Bridge app đang chạy (`rustyclaw peripheral setup-uno-q` triển khai và khởi động nó). Config phải có `board = "arduino-uno-q"` và `transport = "bridge"`.
- **LLM provider (GLM/Zhipu)** — Dùng `default_provider = "glm"` hoặc `"zhipu"` với `GLM_API_KEY` trong env hoặc config. RustyClaw dùng endpoint v4 chính xác.
