...
- `cargo check` / `cargo build` dừng lâu ở `Checking rustyclaw`
...
### Không tìm thấy lệnh `rustyclaw` sau cài đặt

Triệu chứng:

- Cài đặt thành công nhưng shell không tìm thấy `rustyclaw`
...
Xác minh `~/.rustyclaw/config.toml`:
...
- Dừng các tiến trình `rustyclaw daemon` / `rustyclaw channel start` thừa
