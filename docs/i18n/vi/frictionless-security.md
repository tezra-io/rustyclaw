...
[INFO] ✓ Audit logging enabled (~/.config/rustyclaw/audit.log)
...
$ rustyclaw security --status
Security Status:
  ✓ Sandbox: Landlock (Linux kernel 6.2)
  ✓ Memory monitoring: 512MB limit
  ✓ Audit logging: ~/.config/rustyclaw/audit.log
  → 47 events logged today

# Tắt sandbox tường minh (ghi vào config)
$ rustyclaw config set security.sandbox.enabled false

# Bật backend cụ thể
$ rustyclaw config set security.sandbox.backend firejail

# Điều chỉnh giới hạn
$ rustyclaw config set security.resources.max_memory_mb 2048
```
...
```bash
$ rustyclaw onboard --security-level paranoid
[1/9] Workspace Setup...
...
...
✅ **Hiển thị trạng thái** — `rustyclaw security --status`
