# Thêm Board và Tool — Hướng dẫn phần cứng RustyClaw

Hướng dẫn này giải thích cách thêm board phần cứng mới và tool tùy chỉnh vào RustyClaw.

## Bắt đầu nhanh: Thêm board qua CLI

```bash
# Thêm board (cập nhật ~/.rustyclaw/config.toml)
rustyclaw peripheral add nucleo-f401re /dev/ttyACM0
rustyclaw peripheral add arduino-uno /dev/cu.usbmodem12345
rustyclaw peripheral add rpi-gpio native   # cho Raspberry Pi GPIO (Linux)
...
Với feature `rag-pdf`, RustyClaw có thể lập chỉ mục file PDF:
...
2. **Thêm vào config** — `rustyclaw peripheral add my-board /dev/ttyUSB0`
...
| Lệnh | Mô tả |
|------|-------|
| `rustyclaw peripheral list` | Liệt kê các board đã cấu hình |
| `rustyclaw peripheral add <board> <path>` | Thêm board (ghi vào config) |
| `rustyclaw peripheral flash` | Nạp firmware Arduino |
| `rustyclaw peripheral flash-nucleo` | Nạp firmware Nucleo |
| `rustyclaw hardware discover` | Liệt kê thiết bị USB |
| `rustyclaw hardware info` | Thông tin chip qua probe-rs |
