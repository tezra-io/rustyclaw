# Triển khai mạng — RustyClaw trên Raspberry Pi và mạng nội bộ

Tài liệu này hướng dẫn triển khai RustyClaw trên Raspberry Pi hoặc host khác trong mạng nội bộ, với các channel Telegram và webhook tùy chọn.
...
|------|----------------------|----------|
| **Telegram polling** | Không | RustyClaw poll Telegram API; hoạt động từ bất kỳ đâu |
| **Matrix sync (kể cả E2EE)** | Không | RustyClaw sync qua Matrix client API; không cần webhook đến |
...
**Lưu ý:** Telegram, Discord và Slack dùng **long-polling** — RustyClaw thực hiện các request ra ngoài. Không cần port forwarding hoặc public IP.

---

## 2. RustyClaw trên Raspberry Pi
...
Chỉnh sửa `~/.rustyclaw/config.toml`:
...
- Channel Telegram hoạt động: RustyClaw poll Telegram API (outbound)
...
```bash
rustyclaw daemon --host 0.0.0.0 --port 3000
```
...
2. Khởi động tunnel:
   ...
   ```
   Hoặc dùng `rustyclaw tunnel` (xem tài liệu tunnel).

3. RustyClaw sẽ từ chối `0.0.0.0` trừ khi `allow_public_bind = true` hoặc có tunnel đang hoạt động.
...
- RustyClaw gọi `https://api.telegram.org/bot{token}/getUpdates`
...
Chạy `rustyclaw daemon` — channel Telegram khởi động tự động.
...
- Chỉ chạy một instance runtime cho cùng token (khuyến nghị: service `rustyclaw daemon`).
...
- [ ] Chạy `rustyclaw daemon --host 127.0.0.1 --port 3000` (Telegram hoạt động không cần 0.0.0.0)
