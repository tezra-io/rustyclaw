# RustyClaw Config Reference (Operator-Oriented)
...
1. `RUSTYCLAW_WORKSPACE` override (if set)
2. persisted `~/.rustyclaw/active_workspace.toml` marker (if present)
3. default `~/.rustyclaw/config.toml`

RustyClaw logs the resolved config on startup at `INFO` level:

- `Config loaded` with fields: `path`, `workspace`, `source`, `initialized`

Schema export command:

- `rustyclaw config schema` (prints JSON Schema draft 2020-12 to stdout)
...
| `otel_service_name` | `rustyclaw` | Service name emitted to OTLP collector |
...
- Query runtime traces with:
  - `rustyclaw doctor traces --limit 20`
  - `rustyclaw doctor traces --event tool_call_result --contains \"error\"`
  - `rustyclaw doctor traces --id <trace-id>`
...
1. `RUSTYCLAW_PROVIDER` (explicit override, always wins when non-empty)
...
- Use `RUSTYCLAW_PROVIDER` when you intentionally want runtime env to override a non-default configured provider.
...
- When `enabled = true` and no OTP secret exists, RustyClaw generates one and prints an enrollment URI once.
...
| `state_file` | `~/.rustyclaw/estop-state.json` | Persistent estop state path |
...
- Use CLI command `rustyclaw estop` to engage and `rustyclaw estop resume` to clear levels.
...
- Security-first default: RustyClaw does **not** clone or sync `open-skills` unless `open_skills_enabled = true`.
- Environment overrides:
  - `RUSTYCLAW_OPEN_SKILLS_ENABLED` accepts `1/0`, `true/false`, `yes/no`, `on/off`.
  - `RUSTYCLAW_OPEN_SKILLS_DIR` overrides the repository path when non-empty.
  - `RUSTYCLAW_SKILLS_PROMPT_MODE` accepts `full` or `compact`.
- Precedence for enable flag: `RUSTYCLAW_OPEN_SKILLS_ENABLED` → `skills.open_skills_enabled` in `config.toml` → default `false`.
...
- Skill loading and `rustyclaw skills install` both apply a static security audit. Skills that contain symlinks, script-like files, high-risk shell payload snippets, or unsafe markdown link traversal are rejected.
...
- RustyClaw requests Composio v3 tools with `toolkit_versions=latest` and executes tools with `version="latest"` to avoid stale default tool revisions.
...
3. Validate with `rustyclaw doctor` before restart/rollout.
...
- While `rustyclaw channel start` is running, updates to `default_provider`, `default_model`, `default_temperature`, `api_key`, `api_url`, and `reliability.*` are hot-applied from `config.toml` on the next inbound message.
...
- `RUSTYCLAW_LINQ_SIGNING_SECRET` overrides `signing_secret` when set.
...
- `RUSTYCLAW_NEXTCLOUD_TALK_WEBHOOK_SECRET` overrides `webhook_secret` when set.
...
| Key | Default | Purpose |
|---|---|---|
| `base_url` | _auto-resolved_ | Override URL for the Elixir orchestrator API (e.g. `http://127.0.0.1:50191`) |

When `base_url` is not set, the daemon resolves the port from `RUSTYCLAW_ELIXIR_SYNTH_PORT` (default `50191`).

### Elixir Bridge Environment Variables

| Variable | Default | Purpose |
|---|---|---|
| `RUSTYCLAW_BRIDGE_SECRET` | _auto-generated by daemon_ | Shared secret for Rust ↔ Elixir API authentication |
| `RUSTYCLAW_ELIXIR_SYNTH_PORT` | `50191` | Port for the Elixir tool synthesis / agent API |
| `RUSTYCLAW_ELIXIR_PLUGIN_PORT` | `50192` | Port for the Elixir plugin API |

Notes:

- The daemon auto-generates an ephemeral bridge secret at startup when `RUSTYCLAW_BRIDGE_SECRET` is not set.
- All `/api/agents/*` routes require the bridge secret via the `x-bridge-secret` header.
- When the orchestrator is not running, agent bridge tools (`spawn_agent`, `list_agents`, `message_agent`, `kill_agent`, `delegate_agent`) degrade with a clear "not reachable" tool error.

```toml
[elixir]
base_url = "http://127.0.0.1:50191"
```
