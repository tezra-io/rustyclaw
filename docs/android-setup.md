...
RustyClaw provides prebuilt binaries for Android devices.
...
The easiest way to run RustyClaw on Android is via [Termux](https://termux.dev/).
...
# For 64-bit (aarch64):
curl -LO https://github.com/tezra-io/rustyclaw/releases/latest/download/rustyclaw-aarch64-linux-android.tar.gz
tar xzf rustyclaw-aarch64-linux-android.tar.gz

# For 32-bit (armv7):
curl -LO https://github.com/tezra-io/rustyclaw/releases/latest/download/rustyclaw-armv7-linux-androideabi.tar.gz
tar xzf rustyclaw-armv7-linux-androideabi.tar.gz
```
...
For advanced users who want to run RustyClaw outside Termux:

```bash
# From your computer with ADB
adb push rustyclaw /data/local/tmp/
adb shell chmod +x /data/local/tmp/rustyclaw
adb shell /data/local/tmp/rustyclaw --version
```
