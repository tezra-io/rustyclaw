# Thiết kế Hardware Peripherals — RustyClaw

RustyClaw cho phép các vi điều khiển (MCU) và máy tính nhúng (SBC) **phân tích lệnh ngôn ngữ tự nhiên theo thời gian thực**, tổng hợp code phù hợp với từng phần cứng, và thực thi tương tác với ngoại vi trực tiếp.

## 1. Tầm nhìn

**Mục tiêu:** RustyClaw đóng vai trò là AI agent có hiểu biết về phần cứng, cụ thể:
...
**Hình dung trực quan:** RustyClaw = bộ não hiểu phần cứng. Ngoại vi = tay chân mà nó điều khiển.
...
RustyClaw chạy **trực tiếp trên thiết bị**. Board khởi động server gRPC/nanoRPC và giao tiếp với ngoại vi ngay tại chỗ.

```
┌─────────────────────────────────────────────────────────────────────────────┐
│  RustyClaw on ESP32 / Raspberry Pi (Edge-Native)                             │
│                                                                             │
│  ┌─────────────┐    ┌──────────────┐    ┌─────────────────────────────────┐ │
...
2. RustyClaw truy xuất tài liệu theo board (ví dụ: bản đồ GPIO của ESP32)
...
RustyClaw chạy trên **máy chủ** và duy trì kết nối phần cứng tới thiết bị mục tiêu. Dùng cho phát triển, kiểm tra nội tâm, và nạp firmware.

```
┌─────────────────────┐                    ┌──────────────────────────────────┐
│  RustyClaw on Mac    │   USB / J-Link /   │  STM32 Nucleo-F401RE              │
...
2. RustyClaw nhận diện phần cứng đang kết nối (VID/PID, kiến trúc)
...
2. RustyClaw ghi/nạp firmware qua OpenOCD hoặc probe-rs
3. Xác nhận thành công

**Hoặc:**
1. RustyClaw tự phát hiện: *"STM32 Nucleo on /dev/ttyACM0, ARM Cortex-M4"*
...
|-----------|-------------|---------------|
| RustyClaw chạy trên | Thiết bị (ESP32, RPi) | Máy chủ (Mac, Linux) |
...
Máy chủ chạy RustyClaw; ngoại vi chạy firmware tối giản. JSON đơn giản qua serial.

### Chế độ B: RPi làm Host (Native GPIO)

RustyClaw trên Pi; GPIO qua rppal hoặc sysfs. Không cần firmware riêng.
...
rustyclaw agent --mode edge

# Host-Mediated: connect to USB/J-Link target
rustyclaw agent --peripheral nucleo-f401re:/dev/ttyACM0
rustyclaw agent --probe jlink

# Hardware introspection
rustyclaw hardware discover
rustyclaw hardware introspect /dev/ttyACM0
```
...
board = "esp32"
transport = "wifi"
# Edge-Native: RustyClaw runs on ESP32
```
...
1. **Khởi động:** RustyClaw nạp config, đọc `peripherals.boards`.
...
Dành cho RPC có kiểu dữ liệu, độ trễ thấp giữa RustyClaw và các peripheral:
...
- **rustyclaw-firmware** hoặc **rustyclaw-peripheral** — một crate/workspace riêng biệt.
...
- Người dùng nạp lên board; RustyClaw kết nối và tự phát hiện khả năng.

## 9. Các giai đoạn triển khai

### Phase 1: Skeleton ✅ (Hoàn thành)

- [x] Thêm trait `Peripheral`, config schema, CLI (`rustyclaw peripheral list/add`)
- [x] Thêm flag `--peripheral` cho agent
- [x] Ghi tài liệu vào AGENTS.md

### Phase 2: Host-Mediated — Phát hiện phần cứng ✅ (Hoàn thành)

- [x] `rustyclaw hardware discover`: liệt kê thiết bị USB (VID/PID)
- [x] Board registry: ánh xạ VID/PID → kiến trúc, tên (ví dụ: Nucleo-F401RE)
- [x] `rustyclaw hardware introspect <path>`: memory map, danh sách peripheral
...
- [x] RustyClaw trên Raspberry Pi (native GPIO qua rppal)
...
- [x] Crate firmware `rustyclaw-esp32` (`firmware/rustyclaw-esp32`) — GPIO qua UART
- [x] ESP32 trong hardware registry (CH340 VID/PID)
- [ ] RustyClaw *chạy trực tiếp trên* ESP32 (WiFi + LLM, edge-native) — tương lai
- [ ] Thực thi Wasm hoặc dựa trên template cho logic do LLM tạo ra

**Cách dùng:** Nạp `firmware/rustyclaw-esp32` vào ESP32, thêm `board = "esp32"`, `transport = "serial"`, `path = "/dev/ttyUSB0"` vào config.
...
- Chạy RustyClaw đầy đủ *trực tiếp trên* STM32 bare-metal (không có WiFi, RAM hạn chế) — dùng Host-Mediated thay thế
...
> *"Các board như ESP, Raspberry Pi, hoặc các board có WiFi có thể kết nối với LLM (Gemini hoặc mã nguồn mở). RustyClaw chạy trên thiết bị, tạo gRPC riêng, khởi động nó, và giao tiếp với ngoại vi. Người dùng hỏi qua WhatsApp: 'di chuyển cánh tay X' hoặc 'bật LED'. RustyClaw lấy tài liệu chính xác, viết code, thực thi, lưu trữ tối ưu, chạy, và bật LED — tất cả trên board phát triển.*
>
> *Với STM Nucleo kết nối qua USB/J-Link/Aardvark vào Mac: RustyClaw từ Mac truy cập phần cứng, cài đặt hoặc ghi những gì cần thiết lên thiết bị, và trả kết quả. Ví dụ: 'Hey RustyClaw, những địa chỉ khả dụng/đọc được trên thiết bị USB này là gì?' Nó có thể tự tìm ra thiết bị nào đang kết nối ở đâu và đưa ra gợi ý."*
