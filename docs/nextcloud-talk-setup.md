...
This guide covers native Nextcloud Talk integration for RustyClaw.
...
Add this section in `~/.rustyclaw/config.toml`:
...
rustyclaw daemon
# or
rustyclaw gateway --host 127.0.0.1 --port 3000
```
...
When `webhook_secret` is configured, RustyClaw verifies:
...
- RustyClaw ignores bot-originated webhook events (`actorType = bots`).
- RustyClaw ignores non-message/system events.
...
3. Confirm RustyClaw receives and replies in the same room.
