#!/bin/bash
# RustyClaw Telegram Integration Test Suite
...
if [ -f "target/release/rustyclaw" ]; then
    BINARY_SIZE=$(ls -lh target/release/rustyclaw | awk '{print $5}')
    SIZE_BYTES=$(stat -f%z target/release/rustyclaw 2>/dev/null || stat -c%s target/release/rustyclaw)
...
CONFIG_PATH="$HOME/.rustyclaw/config.toml"
if [ -f "$CONFIG_PATH" ]; then
    ...
    if grep -q "\[channels_config.telegram\]" "$CONFIG_PATH"; then
        ...
        warn "Telegram not configured - run 'rustyclaw onboard' first"
    fi
else
    warn "No config file found - run 'rustyclaw onboard' first"
fi
...
START_TIME=$(date +%s)
HEALTH_OUTPUT=$(timeout 10 target/release/rustyclaw channel doctor 2>&1 || true)
END_TIME=$(date +%s)
