...
2. **GitHub**: Use [GitHub Security Advisories](https://github.com/theonlyhennygod/rustyclaw/security/advisories/new)
...
RustyClaw implements defense-in-depth security:
...
RustyClaw Docker images follow CIS Docker Benchmark best practices:
...
docker inspect --format='{{.Config.User}}' rustyclaw
# Expected: 65534:65534

# Run with read-only filesystem (production hardening)
docker run --read-only -v /path/to/workspace:/workspace rustyclaw gateway
```
