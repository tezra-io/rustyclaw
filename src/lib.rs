...
Examples:
  ...
  rustyclaw channel add discord '{\"bot_token\":\"...\",\"name\":\"my-discord\"}'")]
    Add {
...
Examples:
  ...
  rustyclaw cron add-at 2025-12-31T23:59:00Z 'Happy New Year!'")]
    AddAt {
...
Examples:
  rustyclaw cron add-every 60000 'Ping heartbeat'     # every minute
  rustyclaw cron add-every 3600000 'Hourly report'    # every hour")]
    AddEvery {
...
Examples:
  ...
  rustyclaw cron once 2h 'Follow up on deployment'
  rustyclaw cron once 1d 'Daily check'")]
    Once {
        /// Delay duration
        delay: String,
        /// Command to run
...
Examples:
  rustyclaw cron update <task-id> --expression '0 8 * * *'
  rustyclaw cron update <task-id> --tz Europe/London --name 'Morning check'
  rustyclaw cron update <task-id> --command 'Updated message'")]
    Update {
        /// Task ID
        id: String,
        /// New cron expression
...
Examples:
  ...
  rustyclaw hardware introspect COM3")]
    Introspect {
...
Examples:
  ...
  rustyclaw peripheral add esp32 /dev/ttyUSB0")]
    Add {
