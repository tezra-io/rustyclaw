//! Flash RustyClaw Nucleo-F401RE firmware via probe-rs.
//!
...
/// Flash RustyClaw Nucleo firmware. Builds from firmware/rustyclaw-nucleo.
pub fn flash_nucleo_firmware() -> Result<()> {
    ...
    // CARGO_MANIFEST_DIR = repo root (rustyclaw's Cargo.toml)
    let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let firmware_dir = repo_root.join("firmware").join("rustyclaw-nucleo");
    if !firmware_dir.join("Cargo.toml").exists() {
        anyhow::bail!(
            "Nucleo firmware not found at {}. Run from rustyclaw repo root.",
            firmware_dir.display()
        );
    }

    println!("Building RustyClaw Nucleo firmware...");
    ...
    println!("RustyClaw Nucleo firmware flashed successfully.");
