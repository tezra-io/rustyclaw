//! Deploy RustyClaw Bridge app to Arduino Uno Q.

use anyhow::{Context, Result};
use std::process::Command;

const BRIDGE_APP_NAME: &str = "rustyclaw-uno-q-bridge";
...
pub fn setup_uno_q_bridge(host: Option<&str>) -> Result<()> {
    ...
    if let Some(h) = host {
        if bridge_dir.exists() {
            ...
            anyhow::bail!(
                "Bridge app not found at {}. Run from rustyclaw repo root.",
                bridge_dir.display()
            );
        }
    } else {
...
fn deploy_remote(host: &str, bridge_dir: &std::path::Path) -> Result<()> {
    ...
    println!("RustyClaw Bridge app started. Add to config.toml:");
...
fn deploy_local(bridge_dir: Option<&std::path::Path>) -> Result<()> {
    ...
    println!("RustyClaw Bridge app started.");
    Ok(())
}

fn write_embedded_bridge(dest: &std::path::Path) -> Result<()> {
    let app_yaml = include_str!("../../firmware/rustyclaw-uno-q-bridge/app.yaml");
