...
/// Resolve the agents directory from the rustyclaw home dir.
/// `agents_dir` = `~/.rustyclaw/agents/` (sibling of workspace/, NOT inside it)
pub fn agents_dir_from_config(config: &crate::config::Config) -> std::path::PathBuf {
    // config.workspace_dir = ~/.rustyclaw/workspace/
    // agents_dir = ~/.rustyclaw/agents/
    config
        .workspace_dir
        .parent()
...
"#;
    ...
    fn agents_dir_is_sibling_of_workspace() {
        let config = crate::config::Config {
            workspace_dir: std::path::PathBuf::from("/home/user/.rustyclaw/workspace"),
            ..crate::config::Config::default()
        };
        let dir = agents_dir_from_config(&config);
        assert_eq!(
            dir,
            std::path::PathBuf::from("/home/user/.rustyclaw/agents")
        );
    }
