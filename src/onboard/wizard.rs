...
const BANNER: &str = r"
    ...
    Your AI Agent Runtime. Pure Rust. Fully Agnostic.
...
pub async fn run_wizard(force: bool) -> Result<Config> {
    ...
    println!(
        "  {}",
        style("Welcome to RustyClaw — the fastest, smallest AI assistant.")
            .white()
            .bold()
    );
    println!(
    ...
    let mut config = Config {
        ...
        elixir: crate::config::ElixirConfig::default(),
    };
    ...
    if has_channels && config.api_key.is_some() {
        ...
        if launch {
            ...
            std::env::set_var("RUSTYCLAW_AUTOSTART_CHANNELS", "1");
        }
    }
...
/// Use `rustyclaw onboard` or `rustyclaw onboard --api-key sk-... --provider openrouter --memory sqlite|lucid`.
/// Use `rustyclaw onboard --interactive` for the full wizard.
...
fn resolve_quick_setup_dirs_with_home(home: &Path) -> (PathBuf, PathBuf) {
    ...
    let config_dir = home.join(".rustyclaw");
...
async fn run_quick_setup_with_home(
    ...
    let (rustyclaw_dir, workspace_dir) = resolve_quick_setup_dirs_with_home(home);
    ...
    if credential_override.is_none() {
        if provider_supports_keyless_local_usage(&provider_name) {
            println!("    1. Chat:     rustyclaw agent -m \"Hello!\"");
            println!("    2. Gateway:  rustyclaw gateway");
            ...
            if canonical_provider_name(&provider_name) == "copilot" {
                ...
                println!(
                    "    1. Login:             rustyclaw auth login --provider {}",
                    provider_name
                );
                println!("    2. Chat:              rustyclaw agent -m \"Hello!\"");
                println!("    3. Gateway:           rustyclaw gateway");
                println!("    4. Status:            rustyclaw status");
            }
        } else {
            let env_var = provider_env_var(&provider_name);
            println!("    1. Set your API key:  export {env_var}=\"sk-...\"");
            println!("    2. Or edit:           ~/.rustyclaw/config.toml");
            println!("    3. Chat:              rustyclaw agent -m \"Hello!\"");
            println!("    4. Gateway:           rustyclaw gateway");
        }
    } else {
        println!("    1. Chat:     rustyclaw agent -m \"Hello!\"");
        println!("    2. Gateway:  rustyclaw gateway");
...
pub async fn run_models_refresh(
    ...
    if !force {
        ...
        .await?
        {
            ...
            println!();
            println!(
                "Tip: run `rustyclaw models refresh --force --provider {}` to fetch latest now.",
                provider_name
            );
            return Ok(());
        }
...
pub async fn run_models_list(config: &Config, provider_override: Option<&str>) -> Result<()> {
    ...
    let Some(cached) = cached else {
        println!();
        println!(
            "  No cached models for '{provider_name}'. Run: rustyclaw models refresh --provider {provider_name}"
        );
        println!();
        return Ok(());
    };
...
async fn setup_provider(workspace_dir: &Path) -> Result<(String, String, String, Option<String>)> {
    ...
    let api_key = if provider_name == "ollama" {
        ...
        if crate::providers::gemini::GeminiProvider::has_cli_credentials() {
            ...
            print_bullet("RustyClaw will reuse your existing Gemini CLI authentication.");
            println!();
...
fn setup_tool_mode() -> Result<(ComposioConfig, SecretsConfig)> {
    ...
    let composio_config = if choice == 1 {
        ...
        print_bullet("RustyClaw uses Composio as a tool — your core agent stays local.");
        println!();
    ...
    // ── Encrypted secrets ──
    println!();
    print_bullet("RustyClaw can encrypt API keys stored in config.toml.");
...
fn setup_hardware() -> Result<HardwareConfig> {
    ...
    let choice = Select::new()
        .with_prompt("  How should RustyClaw interact with the physical world?")
        .items(&options)
        .default(recommended)
        .interact()?;
        ...
        match choice {
            ...
            ChannelMenuChoice::Discord => {
                ...
                println!(
                    ...
                    style("— talk to RustyClaw from Discord").dim()
                );
            ...
            ChannelMenuChoice::Slack => {
                ...
                println!(
                    ...
                    style("— talk to RustyClaw from Slack").dim()
                );
            ...
            ChannelMenuChoice::IMessage => {
                ...
                print_bullet("RustyClaw reads your iMessage database and replies via AppleScript.");
                print_bullet(
            ...
            ChannelMenuChoice::WhatsApp => {
                ...
                if mode_idx == 0 {
                    ...
                    let session_path: String = Input::new()
                        ...
                        .default("~/.rustyclaw/state/whatsapp-web/session.db".into())
                        .interact_text()?;
                ...
                let verify_token: String = Input::new()
                    ...
                    .default("rustyclaw-whatsapp-verify".into())
                    .interact_text()?;
                ...
                config.whatsapp = Some(WhatsAppConfig {
                    ...
                    app_secret: None, // Can be set via RUSTYCLAW_WHATSAPP_APP_SECRET env var
                    session_path: None,
                    pair_phone: None,
                    pair_code: None,
                    allowed_numbers,
            ...
            ChannelMenuChoice::Lark | ChannelMenuChoice::Feishu => {
                ...
                println!(
                    ...
                    style(format!("— talk to RustyClaw from {provider_label}")).dim()
                );
                print_bullet(&format!(
            ...
            ChannelMenuChoice::Nostr => {
                ...
                print_bullet("RustyClaw will listen for encrypted DMs on Nostr relays.");
...
fn print_summary(config: &Config) {
    ...
    println!(
        "  {}  {}",
        style("⚡").cyan(),
        style("RustyClaw is ready!").white().bold()
    );
    println!(
        "  {}",
        style("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━").cyan()
    ...
    if config.api_key.is_none() && !provider_supports_keyless_local_usage(provider) {
        if provider == "openai-codex" {
            ...
            println!(
                "       {}",
                style("rustyclaw auth login --provider openai-codex --device-code").yellow()
            );
            ...
            println!(
                "       {}",
                style(
                    "or: rustyclaw auth paste-token --provider anthropic --auth-kind authorization"
                )
                .yellow()
            );
        } else {
    ...
    if has_channels {
        ...
        println!("       {}", style("rustyclaw channel start").yellow());
        println!();
        step += 1;
    }
    ...
    println!(
        "       {}",
        style("rustyclaw agent -m \"Hello, RustyClaw!\"").yellow()
    );
    println!();
    step += 1;
    ...
    println!("       {}", style("rustyclaw agent").yellow());
    println!();
    step += 1;
    ...
    println!("       {}", style("rustyclaw status").yellow());
...
mod tests {
    ...
    async fn quick_setup_existing_config_requires_force_when_non_interactive() {
        ...
        let _config_env = EnvVarGuard::unset("RUSTYCLAW_CONFIG_DIR");
        let tmp = TempDir::new().unwrap();
        let rustyclaw_dir = tmp.path().join(".rustyclaw");
        let config_path = rustyclaw_dir.join("config.toml");

        tokio::fs::create_dir_all(&rustyclaw_dir).await.unwrap();
    ...
    #[tokio::test]
    async fn quick_setup_respects_rustyclaw_workspace_env_layout() {
        let _env_guard = env_lock().lock().await;
        let tmp = TempDir::new().unwrap();
        let workspace_root = tmp.path().join("rustyclaw-data");
        let workspace_dir = workspace_root.join("workspace");
        let expected_config_path = workspace_root.join(".rustyclaw").join("config.toml");
        ...
        .await
        .expect("quick setup should honor RUSTYCLAW_WORKSPACE");
    ...
    async fn scaffold_uses_defaults_for_empty_context() {
        ...
        let identity = tokio::fs::read_to_string(tmp.path().join("IDENTITY.md"))
            ...
            .unwrap();
        assert!(
            identity.contains("**Name:** RustyClaw"),
            "should default agent name to RustyClaw"
        );
    ...
    async fn scaffold_handles_special_characters_in_names() {
        ...
        let soul = tokio::fs::read_to_string(tmp.path().join("SOUL.md"))
            .await
            .unwrap();
        assert!(soul.contains("RustyClaw-v2"));
    }
