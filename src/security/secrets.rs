...
// in `~/.rustyclaw/.secret_key` with restrictive file permissions (0600). The
...
pub struct SecretStore {
    /// Path to the key file (`~/.rustyclaw/.secret_key`)
    key_path: PathBuf,
...
impl SecretStore {
    ...
    pub fn new(rustyclaw_dir: &Path, enabled: bool) -> Self {
        Self {
            key_path: rustyclaw_dir.join(".secret_key"),
            enabled,
        }
    }
