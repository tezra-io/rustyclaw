...
pub fn run(config: &Config) -> Result<()> {
    ...
    // Print report
    println!("🩺 RustyClaw Doctor (enhanced)");
    println!();
    ...
    if errors > 0 {
        println!("  💡 Fix the errors above, then run `rustyclaw doctor` again.");
    }
...
pub async fn run_models(
    ...
    println!("🩺 RustyClaw Doctor — Model Catalog Probe");
...
fn check_config_semantics(config: &Config, items: &mut Vec<DiagItem>) {
    ...
    if has_channel {
        ...
        items.push(DiagItem::warn(
            cat,
            "no channels configured — run `rustyclaw onboard` to set one up",
        ));
    }
...
mod tests {
    ...
    fn workspace_probe_path_is_hidden_and_unique() {
        ...
        assert!(first
            ...
            .is_some_and(|name| name.starts_with(".rustyclaw_doctor_probe_")));
    }
