...
/// Tokens are cached to `~/.config/rustyclaw/copilot/` and refreshed
/// automatically.
...
impl CopilotProvider {
    pub fn new(github_token: Option<&str>) -> Self {
        let token_dir = directories::ProjectDirs::from("", "", "rustyclaw")
            ...
            .unwrap_or_else(|| {
                ...
                std::env::temp_dir().join(format!("rustyclaw-copilot-{user}"))
            });
