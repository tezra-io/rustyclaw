                                ...
                                Ok(()) => {
                                    let _ = self
                                        .send(&SendMessage::new(
                                            "✅ Telegram account bound successfully. You can talk to RustyClaw now.",
                                            &chat_id,
                                        ))
                                        .await;
                                    tracing::info!(
...
Allowlist Telegram username (without '@') or numeric user ID.",
    ...
    async fn send_text_chunks(
        ...
        for (index, chunk) in chunks.iter().enumerate() {
            ...
            // Quote-reply on the first chunk only (Telegram reply_parameters)
            if index == 0 {
                if let Some(reply_id) = reply_to_message_id {
                    if let Ok(id) = reply_id.parse::<i64>() {
                        markdown_body["reply_parameters"] = serde_json::json!({"message_id": id});
                    }
                }
            }
            ...
            // Quote-reply on the first chunk only (plain fallback)
            if index == 0 {
                if let Some(reply_id) = reply_to_message_id {
                    if let Ok(id) = reply_id.parse::<i64>() {
                        plain_body["reply_parameters"] = serde_json::json!({"message_id": id});
                    }
                }
            }
...
impl Channel for TelegramChannel {
    ...
    async fn send(&self, message: &SendMessage) -> anyhow::Result<()> {
        ...
        let reply_to = message.quote_reply_id.as_deref();
...
Ensure only one `rustyclaw` process is using this bot token."
                    );
...
mod tests {
    ...
    fn telegram_extract_bind_code_supports_bot_mention() {
        assert_eq!(
            TelegramChannel::extract_bind_code("/bind@rustyclaw_bot 654321"),
            Some("654321")
        );
    }
