...
//! RustyClaw to external platforms. Each channel implements the [`Channel`] trait
...
const SYSTEMD_STATUS_ARGS: [&str; 3] = ["--user", "is-active", "rustyclaw.service"];
const SYSTEMD_RESTART_ARGS: [&str; 3] = ["--user", "restart", "rustyclaw.service"];
const OPENRC_STATUS_ARGS: [&str; 2] = ["rustyclaw", "status"];
const OPENRC_RESTART_ARGS: [&str; 2] = ["rustyclaw", "restart"];
...
async fn load_runtime_defaults_from_config_file(path: &Path) -> Result<ChannelRuntimeDefaults> {
    ...
    if let Some(rustyclaw_dir) = path.parent() {
        let store = crate::security::SecretStore::new(rustyclaw_dir, parsed.secrets.encrypt);
...
fn build_models_help_response(current: &ChannelRouteSelection, workspace_dir: &Path) -> String {
    ...
    if cached_models.is_empty() {
        let _ = writeln!(
            response,
            "\nNo cached model list found for `{}`. Ask the operator to run `rustyclaw models refresh --provider {}`.",
...
pub fn build_system_prompt_with_mode(
    ...
    if prompt.is_empty() {
        "You are RustyClaw, a fast and efficient AI assistant built in Rust. Be helpful, concise, and direct."
            .to_string()
    } else {
        prompt
    }
...
async fn bind_telegram_identity(config: &Config, identity: &str) -> Result<()> {
    ...
    let Some(telegram) = updated.channels_config.telegram.as_mut() else {
        anyhow::bail!(
            "Telegram channel is not configured. Run `rustyclaw onboard --channels-only` first"
        );
    };
    ...
    match maybe_restart_managed_daemon_service() {
        ...
        Ok(false) => {
            println!(
                "ℹ️ No managed daemon service detected. If `rustyclaw daemon`/`channel start` is already running, restart it to load the updated allowlist."
            );
        }
        Err(e) => {
            eprintln!(
                "⚠️ Allowlist saved, but failed to reload daemon service automatically: {e}\n\
                 Restart service manually with `rustyclaw service stop && rustyclaw service start`."
            );
        }
    }
    Ok(())
...
fn maybe_restart_managed_daemon_service() -> Result<bool> {
    if cfg!(target_os = "macos") {
        ...
        let plist = home
            .join("Library")
            .join("LaunchAgents")
            .join("com.rustyclaw.daemon.plist");
        if !plist.exists() {
            return Ok(false);
        }
        ...
        if !listed.contains("com.rustyclaw.daemon") {
            return Ok(false);
        }

        let _ = Command::new("launchctl")
            .args(["stop", "com.rustyclaw.daemon"])
            .output();
        let start_output = Command::new("launchctl")
            .args(["start", "com.rustyclaw.daemon"])
            .output()
    ...
    if cfg!(target_os = "linux") {
        ...
        let openrc_init_script = PathBuf::from("/etc/init.d/rustyclaw");
...
pub(crate) async fn handle_command(command: crate::ChannelCommands, config: &Config) -> Result<()> {
    match command {
        ...
        crate::ChannelCommands::List => {
            ...
            println!("\nTo start channels: rustyclaw channel start");
            println!("To check health:    rustyclaw channel doctor");
            println!("To configure:      rustyclaw onboard");
            Ok(())
        }
        crate::ChannelCommands::Add {
            ...
            anyhow::bail!(
                "Channel type '{channel_type}' — use `rustyclaw onboard` to configure channels"
            );
        }
        crate::ChannelCommands::Remove { name } => {
            anyhow::bail!("Remove channel '{name}' — edit ~/.rustyclaw/config.toml directly");
        }
...
fn collect_configured_channels(
    ...
    if let Some(ref mx) = config.channels_config.matrix {
        channels.push(ConfiguredChannel {
            display_name: "Matrix",
            channel: Arc::new(MatrixChannel::new_with_session_hint_and_rustyclaw_dir(
                mx.homeserver.clone(),
    ...
    if let Some(ref ax) = config.channels_config.axon {
        channels.push(ConfiguredChannel {
            display_name: "Axon",
            channel: Arc::new(AxonChannel::new(
                ax.identity.clone(),
                PathBuf::from(shellexpand::tilde(&ax.broker_socket).as_ref()),
                PathBuf::from(shellexpand::tilde(&ax.keys_dir).as_ref()),
                ax.reconnect_initial_delay_ms,
                ax.allowed_from.clone(),
            )),
        });
    }
...
pub async fn doctor_channels(config: Config) -> Result<()> {
    ...
    if channels.is_empty() {
        println!("No real-time channels configured. Run `rustyclaw onboard` first.");
        return Ok(());
    }

    println!("🩺 RustyClaw Channel Doctor");
    println!();
    ...
    if config.channels_config.webhook.is_some() {
        println!("  ℹ️  Webhook   check via `rustyclaw gateway` then GET /health");
    }
...
pub async fn start_channels(config: Config) -> Result<()> {
    ...
    if channels.is_empty() {
        println!("No channels configured. Run `rustyclaw onboard` to set up channels.");
        return Ok(());
    }

    println!("🦀 RustyClaw Channel Server");
    println!("  🤖 Model:    {model}");
...
BTC is currently around $65,000 based on latest tool output."#
    ...
    async fn process_channel_message_uses_runtime_default_model_from_store() {
        ...
        let runtime_ctx = Arc::new(ChannelRuntimeContext {
            ...
            reliability: Arc::new(crate::config::ReliabilityConfig::default()),
            provider_runtime_options: providers::ProviderRuntimeOptions {
                rustyclaw_dir: Some(temp.path().to_path_buf()),
    ...
    impl Memory for RecallMemory {
        ...
        async fn recall(
            ...
            Ok(vec![crate::memory::MemoryEntry {
                ...
                score: Some(0.9),
                confidence: 1.0,
                source: crate::memory::MemorySource::default(),
    ...
    fn prompt_injects_workspace_files() {
        ...
        assert!(
            prompt.contains("Name: RustyClaw"),
    ...
    fn channel_log_truncation_is_utf8_safe_for_multibyte_text() {
        let msg = "Hello from RustyClaw 🌍. Current status is healthy, and café-style UTF-8 text stays safe in logs.";
...
This is an example JSON object for profile settings."#;
    ...
    fn maybe_restart_daemon_systemd_args_regression() {
        assert_eq!(
            SYSTEMD_STATUS_ARGS,
            ["--user", "is-active", "rustyclaw.service"]
        );
        assert_eq!(
            SYSTEMD_RESTART_ARGS,
            ["--user", "restart", "rustyclaw.service"]
        );
    }

    #[test]
    fn maybe_restart_daemon_openrc_args_regression() {
        assert_eq!(OPENRC_STATUS_ARGS, ["rustyclaw", "status"]);
        assert_eq!(OPENRC_RESTART_ARGS, ["rustyclaw", "restart"]);
    }
    ...
    async fn e2e_photo_attachment_rejected_by_non_vision_provider() {
        ...
        process_channel_message(
            ...
            traits::ChannelMessage {
                ...
                sender: "rustyclaw_user".to_string(),
    ...
    async fn e2e_failed_vision_turn_does_not_poison_follow_up_text_turn() {
        ...
        let histories = runtime_ctx
            ...
            .unwrap_or_else(|e| e.into_inner());
        let turns = histories
            .get("test-channel_rustyclaw_user")
