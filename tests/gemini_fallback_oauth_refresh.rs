...
//!
//! Requires:
//! - Live Gemini OAuth profile in `~/.rustyclaw/auth-profiles.json` with refresh_token
...
async fn gemini_warmup_refreshes_expired_oauth_token() -> Result<()> {
    // Find ~/.rustyclaw/auth-profiles.json
    let home = env::var("HOME").expect("HOME env var not set");
    let rustyclaw_dir = PathBuf::from(home).join(".rustyclaw");
    let auth_profiles_path = rustyclaw_dir.join("auth-profiles.json");
    ...
    if !auth_profiles_path.exists() {
        ...
        eprintln!("Run: rustyclaw auth login --provider gemini");
        return Ok(());
    }
    ...
    let gemini_profile_key = profiles
        ...
        .ok_or_else(|| {
            anyhow::anyhow!(
                "No Gemini OAuth profile found. Run: rustyclaw auth login --provider gemini"
            )
        })?
        .clone();
    ...
    // This will load auth from ~/.rustyclaw/auth-profiles.json (with expired token)
    let provider = rustyclaw::providers::create_provider("gemini", None)?;
