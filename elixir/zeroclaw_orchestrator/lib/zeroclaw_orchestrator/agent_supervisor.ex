defmodule ZeroclawOrchestrator.AgentSupervisor do
  @moduledoc """
  ...
  alias ZeroclawOrchestrator.{AgentDefinition, AgentServer}
  ...
  def stop_agent(agent_name) when is_binary(agent_name) do
    case Registry.lookup(ZeroclawOrchestrator.AgentRegistry, agent_name) do
      [{pid, _}] ->
  ...
  def list_agents do
    ...
    |> Enum.flat_map(fn {_, pid, _, _} ->
      case Registry.keys(ZeroclawOrchestrator.AgentRegistry, pid) do
        [name] -> [name]
        _ -> []
      end
    end)
